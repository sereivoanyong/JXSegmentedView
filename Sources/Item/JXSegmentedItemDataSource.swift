...
open class JXSegmentedItemDataSource: JXSegmentedBaseDataSource {
  ...
  open override func preferredSegmentedView(_ segmentedView: JXSegmentedView, widthForItemAt index: Int) -> CGFloat {
    ...
    case .titleImage(let title, _, _):
      ...
      lazy var titleWidth = width(for: title, height: segmentedView.frame.height, font: itemModel.titleNormalFont)
      switch titleImageType {
      case .leftImage, .rightImage:
        return titleWidth + titleImageSpacing + imageSize.width + itemWidthIncrement
      case .topImage, .bottomImage:
        return max(titleWidth, imageSize.width) + itemWidthIncrement
      case .onlyImage:
        return imageSize.width + itemWidthIncrement
      case .onlyTitle:
        return titleWidth + itemWidthIncrement
